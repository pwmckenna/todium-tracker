...
var onAnnounceRequest = function(req, res) {
	...
	if(query.hasOwnProperty('event') && query['event'] === 'completed') {
		...
		tracker.once('value', function(dataSnapshot) {
			...
			if(value.info_hash !== info_hash) {
				console.error('mismatch info hashes');
				console.log('expected ' + info_hash);
				console.log('got ' + value.info_hash);
				return;
			}

			tracker.child('count').set(value.count + 1);
		});
	}
