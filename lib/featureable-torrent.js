...
var onAnnounceRequest = function(req, res) {
	...
	if(query.hasOwnProperty('event') && query['event'] === 'completed') {
		...
		tracker.once('value', function(dataSnapshot) {
			...
			//update the global amount transferred
			firebase.child('transferred').child((new Date()).toLocaleDateString()).transaction(function(current) {
			...
			updates.transferred = transferred + downloaded;
